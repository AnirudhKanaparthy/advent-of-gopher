...
func (g *Grid) At(x int, y int) (bool, error) {
	if x < 0 || g.width <= x {
		return false, fmt.Errorf("x (%v) is out of range (0, %v)", x, g.width)
	}
	if y < 0 || g.height <= y {
		return false, fmt.Errorf("y (%v) is out of range (0, %v)", y, g.height)
	}
	return g.grid[y*g.height+x], nil
}

func (g *Grid) Set(x int, y int, v bool) error {
	if x < 0 || g.width <= x {
...
func (g *Grid) Toggle(x int, y int) error {
	...
	idx := y*g.height + x
	g.grid[idx] = !(g.grid[idx])
	return nil
}

func (g *Grid) SetRange(on bool, gridRange Range) error {
...
func (g *Grid) Do(instruction Instruction) error {
	...
	case InstructionTurnOn:
		return g.SetRange(true, instruction.gridRange)
	case InstructionTurnOff:
		return g.SetRange(false, instruction.gridRange)
...
func main() {
	...
	fmt.Printf("Number of lights on: %v\n", grid.Count(true))
}
